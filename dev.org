* Current
** TODO Player death causes a crash                                     :bug:
   This requires a game over scene.
*** TODO Create a game over scene
* Backlog
** TODO There is a strange flicker after the fadeout transition
   the stage is rendered with improper camera zoom. I believe this is because
   the viewport resets when a scene transition happens. It probably renders for
   a second like that.  Should think of another way to handle these...
** TODO After the first non-training battle, game breaks                :bug:
** TODO Update the text library 
   to not use bible
** TODO Look into importing sprites from AESPRITE or other sprite generation tool
** TODO Split event constants into own file constants/events.js
** TODO Don't show skills unless in offensive mode
   This should be done on a branch/ as an experiment.  I want to see if it looks
   good, doesn't confuse the player, and it more effectively lets the player
   know that they're attacking.
...
** TODO Some sort of battle won/lost alert/ui thingy
** :release_marker: v0.4
** TODO Pretty up the TWHighlight component
   So, now it does the box around it, but it doesn't bring it into the 
   foreground ahead of the modal mask.  This is a bit awkward. See if we can fix
   that. I'm thinking, we need to duplicate the actual icon, and place the dupe
   inside the highlight box
...
** TODO Battle intros
   Some like, animations the characters do.  Some ui elements 
   ("round one, fight!", "Heaven or hell, let's rock!", etc)
** TODO Host the assets on cloudfront
** TODO Create a deploy script
...
* Done
** DONE Style/polish the tutorial presentation
   CLOSED: [2018-05-19 Sat 17:46]
*** DONE Add some text telling the player they won't die even if they take fatal damage on training scene 2
    CLOSED: [2017-07-04 Tue 15:29]
*** DONE Add scene transitions after each training stage and battle
    CLOSED: [2018-05-19 Sat 17:10]
*** DONE Style the modal.
    CLOSED: [2018-05-19 Sat 17:40]
    Typography, spacing, border, etc.
*** DONE Style the highlight 
    CLOSED: [2018-05-19 Sat 17:46]
    When a UI element/concept gets introduced to the user
** DONE Deploy the app to herkou
   CLOSED: [2017-07-05 Wed 15:13]
** DONE Cleanup Dockerfile of deploy needs
   CLOSED: [2017-07-05 Wed 18:31]
** DONE Tutorial section                                               :epic:
   CLOSED: [2017-06-15 Thu 18:47]
...
**** DONE Scene transition from the first training scene
     ...
     etc etc
*** DONE Training dummy stage 3
    CLOSED: [2017-06-15 Thu 18:39]
